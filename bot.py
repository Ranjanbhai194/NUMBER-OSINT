...
# APIs स्वैप
NUMBER_API_URL = "https://anurixx-gift-number.vercel.app/api"          # /num → Special
NUMBER_SPECIAL_URL = "https://num-info-redzone.susxbunny.workers.dev/api"  # /special → Normal
...
# ---------- API FUNCTIONS (Swapped) ----------
def fetch_number(num):
    """/num → Special API (anurixx)"""
    try:
        url = f"{NUMBER_API_URL}?num={num}"
...
def fetch_number_special(phone):
    """/special → Normal API (num-info)"""
    try:
        url = f"{NUMBER_SPECIAL_URL}?key={NUMBER_API_KEY}&number={phone}"
        r = requests.get(url, timeout=10)
        if r.status_code == 200:
            return r.json()
        return None
...
"""
        ...
        # Normal API (now handled by /special)
        if not data:
            return "`❌ No data`"
        if isinstance(data, dict):
...
"""
        ...
        # Special API (now handled by /num)
...
if __name__ == "__main__":
    ...
    print(f"👨‍💻 {OWNER}")
    print("✅ /num → Special API (anurixx), /special → Normal API (num-info)")
